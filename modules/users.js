...
router.post("/users/login", async function(req, res, next) {
    ...
    try {       
		...
		if (data.rows.length > 0) {
			...
			//retrieve data for the user in the database
            ...
            //verify the password
            let pswCheck = authUtils.verifyPassword(cred.password, hash, salt);
			...
			res.status(403).json({error: "The user doesn't exists"}).end();
            return;
		}		
	}
	catch(err) {
